...
int main() {
	...
	if(vm_write(kernel, pid2, (char *)(42), 1, temp_buf) == 0)
		score += 1;
		printf("Check point 6: Verify wriiting/reading page 1 \n current score = %d\n", score);
	temp_buf[0] = '\0'; // Clean the first byte of temp_buf.
	if(vm_read(kernel, pid2, (char *)(42), 1, temp_buf) == 0)
		score += 1;
		printf("Check point 7: Verify wriiting/reading page 1 \n current score = %d\n", score);
	if(temp_buf[0] == 'a')
		score += 1;
		printf("Check point 8: Verify wriiting/reading page 1 \n current score = %d\n", score);
	get_kernel_free_space_info(kernel, buf);
	if(strcmp(buf, "free space: (addr:288, size:7904)\n") == 0)
		score += 1;
		printf("Check point 9: get_kernel_free_space_info \n current score = %d\n", score);
	...
	if(strcmp(buf, "free space: (addr:416, size:7776)\n") == 0)
		score += 1;
	printf("Check point 11: current score = %d\n", score);

	/*
	for(int i=0; i<20;i++)
	{
		printf("occupy[%d] = %d\n", i,kernel->occupied_pages[i]);
	}
	*/
	...
	if(proc_exit_vm(kernel, pid2) == 0)
		score += 1;
		printf("Check point 12: Exit process 2\n current score = %d\n", score);
