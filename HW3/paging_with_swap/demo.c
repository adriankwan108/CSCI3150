...
int main() {
	...
	if(strcmp(buf, "free space: (addr:256, size:0)\n") == 0)
		score += 1;
	...
	printf("The memory mappings of process %d\n", pid1);
	print_memory_mappings(kernel, pid1);
	...
	if(proc_exit_vm(kernel, pid2) == 0)
		score += 1;
		printf("Checkpoint 12: score = %d\n", score);
	printf("After process 2 exits\n");
	print_kernel_free_space(kernel);
	print_kernel_lru(kernel);
	printf("\n");
	get_kernel_free_space_info(kernel, buf);
	if(strcmp(buf, "free space: (addr:0, size:32) -> (addr:256, size:0)\n") == 0)
		score += 1;
		printf("Checkpoint 13: score = %d\n", score);
	// Exit process 3 and check free space.
	if(proc_exit_vm(kernel, pid3) == 0)
		score += 1;
		printf("Checkpoint 14: score = %d\n", score);
	printf("After process 3 exits\n");
	print_kernel_free_space(kernel);
	print_kernel_lru(kernel);
	printf("\n");
	get_kernel_free_space_info(kernel, buf);
	if(strcmp(buf, "free space: (addr:0, size:160) -> (addr:256, size:0)\n") == 0)
		score += 1;
		printf("Checkpoint 15: score = %d\n", score);

	// Exit process 1 and check free space.
	if(proc_exit_vm(kernel, pid1) == 0)
		score += 1;
		printf("Checkpoint 16: score = %d\n", score);
	printf("After process 1 exits\n");
	print_kernel_free_space(kernel);
	print_kernel_lru(kernel);
	printf("\n");
	get_kernel_free_space_info(kernel, buf);
	if(strcmp(buf, "free space: (addr:0, size:256)\n") == 0)
		score += 1;
		printf("Checkpoint 17: score = %d\n", score);
	// Exit process 0 and check free space.
	if(proc_exit_vm(kernel, pid0) == 0)
		score += 1;
		printf("Checkpoint 18: score = %d\n", score);
